...
public class Enemy extends GameObject{
	...
	public void tick() {
		...
		if(y<=-100 || y>=Game.HEIGHT+100 || x<=-100 || x>=Game.WIDTH+100) handler.removeObject(this);
	}
	...
	public void pattern1() {
		...
		for (int i = 0; i <= bullets; i++) {
			handler.addObject(new EnemyBullet(x, y, mp - i, velY(mp-i, velZ), ID.EnemyBullet, "resources/Bullet.png", handler));
	...
	public void pattern3() {
		...
		for (int i = 0; i <= bullets; i++) {
			for (int j = 0; j < 3; j++) {
					handler.addObject(new EnemyBullet(x, y, mp-i, velY(mp-i, velZ+j), ID.EnemyBullet, "resources/Bullet.png", handler));
			}
		}
	}
	...
	public void pattern4() {
		...
		if(counter <= bullets)
			...
			counter = 0;
		handler.addObject(new EnemyBullet(x, y-counter, mp-counter, velY(mp-counter, velZ), ID.EnemyBullet, "resources/Bullet.png", handler));
	}
	...
	public void pattern5() {
		...
		if(counter2 > 0)
			...
			counter = 20;
		handler.addObject(new EnemyBullet(x, y-counter2, mp-counter2, velY(mp-counter2, velZ), ID.EnemyBullet, "resources/Bullet.png", handler));
	}
